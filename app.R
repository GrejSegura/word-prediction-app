                ...
                tabsetPanel(
                        tabPanel(div(style = "font-weight:normal; font-size:12px; color:#2471A3", 'Home'),
                                ...
                                fluidRow(column(6, align="center",
                                        actionButton(style="font-weight:normal; color: #202121; background-color: #D7DDE3; font-size: 16px; text-transform: lowercase","button1",
                                                textOutput('predict1'))),
...
server <- shinyServer(function(input, output){
                        ...
                        if(numberWords == 0){
                                ...
                                for (i in 1:numberWords){
                                        ...
                                        if (nrow(scoreTable) < 5){
                                                ...
                                                return(scoreTable[1, c('nextWord')])
                                        }
                                        return(scoreTable[1, c('nextWord')])
                                }
                        }
                ...
                observeEvent(input$button1,
                        {output$predict1 <- renderText({
                                return(as.character(unlist(predictTable1())))
                        })
                })
#################################################################################################                
                predictTable2 <- reactive({
                        sentence <- input$searchBar
                        wordBreakDown <- unlist(replaceText(sentence))
                        wordBreakDown <- strsplit(wordBreakDown, split = " ")[[1]]
                        numberWords <- length(wordBreakDown)
                        ...
                        if(numberWords == 0){
                                ...
                                for (i in 1:numberWords){
                                        ...
                                        if (nrow(scoreTable) < 5){
                                                addUniGram <- as.data.frame(unigramData[1:(5-nrow(scoreTable)), c('nextWord')])
                                                scoreTable <- as.data.frame(full_join(scoreTable, addUniGram, by = 'nextWord'))
                                                return(scoreTable[2, c('nextWord')])
                                        }
                                        return(scoreTable[2, c('nextWord')])
                                }
                        }
                ...
                observeEvent(input$button2,
                             {output$predict2 <- renderText({
                                     return(as.character(unlist(predictTable2())))
                             })
                })
})                        
